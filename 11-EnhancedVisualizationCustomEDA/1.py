...
def encrypt_message(message):
    """
    Encrypts a message using two-layer encryption:
    1. Generates a random AES key and encrypts it with RSA-OAEP.
    2. Encrypts the message with AES-GCM using the AES key.
    Returns a base64-encoded string containing all encrypted components.
    """
    steps = 3  # Number of steps for progress bar
    with tqdm(total=steps, desc="Encrypting", ncols=100) as pbar:
        # Step 1: Load or generate RSA keys
        private_key, public_key = load_or_generate_keys()
        pbar.update(1)
        
        # Step 2: Generate AES key and encrypt it with RSA-OAEP
...
def decrypt_message(encoded_data):
    """
    Decrypts a message from a base64-encoded string:
    1. Decrypts the AES key using RSA-OAEP.
    2. Decrypts the message with AES-GCM using the decrypted AES key.
    ...
    with tqdm(total=steps, desc="Decrypting", ncols=100) as pbar:
        ...
        # Step 3: Decrypt AES key and then the message
